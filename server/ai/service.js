...
async function processPhotoAI({ fileBuffer, filename, metadata, gps, device, isRecheck = false, collectibleOverride = null }, modelOverrides = {}) {
  ...
  if (ext === '.heic' || ext === '.heif') {
    ...
    if (allowDevDebug && process.env.NODE_ENV !== 'production') {
      try {
        const os = require('os');
        // SECURITY: Create secure temporary directory with owner-only permissions (0o700)
        // Combined with openSync 'wx' (exclusive) and 0o600, prevents race conditions.
...
async function updatePhotoAIMetadata(db, photoRow, storagePath, modelOverrides = {}, options = {}) {
  try {
    ...
    try {
      ...
      // SECURITY: Validate the signed URL is from our trusted Supabase domain
      // This prevents potential SSRF attacks by ensuring we only fetch from known origins.
      // The URL is generated by Supabase's createSignedUrl API, but we validate anyway.
      ...
      // Parse and validate the URL origin matches our Supabase project
      // Only validate if we have a configured Supabase URL (skip in test environments)
      ...
      if (supabaseUrl) {
        try {
          ...
          throw new Error(`Invalid signed URL format: ${err.message}`);
        }
      }
