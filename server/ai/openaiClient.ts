...
function createDisabledClient(message: any) {
  return {
    chat: {
      completions: {
        create: () => {
...
if (process.env.NODE_ENV === 'test') {
  ...
  if (typeof OpenAI !== 'function') {
    openai = createDisabledClient('OpenAI client could not be initialized from the installed openai package.');
  } else {
  ...
  if (openai?.chat?.completions?.create) {
    ...
    openai.chat.completions.create = async (params: any, options: any) => {
