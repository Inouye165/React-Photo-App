...
describe('foodPlaces nearbyFoodPlaces', () => {
  it('returns empty array when lat/lon missing', async () => {
    const res: unknown[] = await nearbyFoodPlaces(null, null, 250, { fetch: async (): Promise<FakeFetchResponse> => ({ ok: true, json: async () => ({ results: [] }) }) });
  ...
  it('normalizes and sorts returned restaurants by distance', async () => {
    ...
    const fakeFetch = async (): Promise<FakeFetchResponse> => ({ ok: true, json: async () => ({ status: 'OK', results: result }) });
  ...
  it('expands radius up to FOOD_PLACES_MAX_RADIUS_METERS and returns results when available', async () => {
    const originalStart: string | undefined = process.env.FOOD_PLACES_START_RADIUS_METERS;
    const originalMax: string | undefined = process.env.FOOD_PLACES_MAX_RADIUS_METERS;
    ...
    const fakeFetch = jest.fn((url: string) => {
      const params = new URLSearchParams(url.split('?')[1]);
      const radius: number = Number(params.get('radius'));
      if (radius < 250) {
