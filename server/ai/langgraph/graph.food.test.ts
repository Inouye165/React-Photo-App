...
// File: server/ai/langgraph/graph.food.test.ts
...
describe('food nodes behavior', () => {
  ...
  it('food_location_agent selects nearest and attaches it to state', async () => {
    const fakeCandidates: Array<Record<string, unknown>> = [
    ...
    const state: Record<string, unknown> = { filename: 'photo.jpg', metadata: { latitude: 37.123, longitude: -122.456 }, gpsString: '37.123,-122.456' };
  ...
  it('location_intelligence_agent deterministic selects closest high-rated restaurant for non-food scenes', async () => {
    ...
    const state: Record<string, unknown> = { filename: 'photo.jpg', metadata: {}, gpsString: '37.123,-122.456', coords: { lat: 37.123, lon: -122.456 }, classification: 'scenery' };
  ...
  it('food_location_agent deterministic selects closest high-rated restaurant for food', async () => {
    ...
    const state: Record<string, unknown> = { filename: 'photo.jpg', metadata: {}, gpsString: '37.123,-122.456', coords: { lat: 37.123, lon: -122.456 }, classification: 'food' };
  ...
  it('location_intelligence_agent should skip OSM trails for food classification', async () => {
    ...
    const state: Record<string, unknown> = { classification: 'food', metadata: {}, gpsString: '37.123,-122.456', coords: { lat: 37.123, lon: -122.456 } };
    const res = await location_intelligence_agent(state);
  ...
  it('location_intelligence_agent should run OSM trails for non-food classification', async () => {
    ...
    const state: Record<string, unknown> = { classification: 'scenery', metadata: {}, gpsString: '37.123,-122.456', coords: { lat: 37.123, lon: -122.456 } };
    const res = await location_intelligence_agent(state);
  ...
  it('food_metadata_agent produces finalResult and attaches food info', async () => {
    ...
    const mockParsed: Record<string, unknown> = {
      caption: 'Tasty Dish',
  ...
  const enrichedParsed: Record<string, unknown> = { ...mockParsed, caption: 'Crab Boil at Cajun Crackn Concord', description: 'A big bag of seafood, likely from Cajun Crackn Concord', chosen_place_id: 'p_cajun', restaurant_name: 'Cajun Crackn Concord', restaurant_confidence: 0.9 };
  ...
  it('passes curated nearby_food_places to the LLM when available', async () => {
    ...
    const mockParsed: Record<string, unknown> = { caption: 'Crab Boil', description: 'Yum', dish_name: 'Crab Boil', keywords: ['crab'] };
    openai.chat.completions.create.mockResolvedValueOnce({ choices: [{ message: { content: JSON.stringify(mockParsed) } }] });
    fetchDishNutrition.mockResolvedValueOnce(null);
  ...
  it('deterministic override locks restaurant for food', async () => {
    ...
    const candidate: Record<string, unknown> = { placeId: 'p_merr', name: "Merriman's Kapalua", address: '1 Bay Club Place', source: 'google', rating: 4.7, distanceMeters: 27 };
    nearbyFoodPlaces.mockResolvedValueOnce([candidate]);

    const state: Record<string, unknown> = { filename: 'food.jpg', metadata: {}, gpsString: '20.9984033,-156.6671469', coords: { lat: 20.9984033, lon: -156.6671469 }, classification: 'food' };
    const withPlaces = await food_location_agent(state);
    ...
    const mockParsed: Record<string, unknown> = { caption: 'Clams Dish', description: 'Clams', dish_name: 'Clams', restaurant_name: 'Not Merrimans', restaurant_address: '123 Fake St', restaurant_confidence: 0.2 };
    openai.chat.completions.create.mockResolvedValueOnce({ choices: [{ message: { content: JSON.stringify(mockParsed) } }] });
  ...
  it('ignores low-confidence chosen_place_id', async () => {
    ...
    const mockParsedLowConfidence: Record<string, unknown> = {
      caption: 'Crab Boil',
      description: 'A big bag of seafood',
      dish_name: 'Crab boil',
      cuisine: 'Seafood',
