...
async function identify_collectible(state: Record<string, any>): Promise<Record<string, any>> {
  try {
    ...
    const visualMatches: VisualMatch[] | null = await performVisualSearch(state.imageBase64);
    ...
    if (visualMatches && visualMatches.length > 0) {
      webContext = '\n\n## Web Research Context\n' +
        ...
        visualMatches.map((m: VisualMatch, i: number) => `${i + 1}. ${m.title} - ${m.link}`).join('\n') +
