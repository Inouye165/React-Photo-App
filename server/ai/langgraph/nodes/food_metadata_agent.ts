...
async function food_metadata_agent(state: Record<string, any>): Promise<Record<string, any>> {
  ...
  try {
    ...
    const city: string | null = promptPoi.city || (promptPoi.locationIntel ? promptPoi.locationIntel.city : null) || promptPoi.location || promptPoi.region || null;
    const region: string | null = promptPoi.region || (promptPoi.locationIntel ? promptPoi.locationIntel.region : null) || null;
    const locationString: string = [city, region].filter(Boolean).join(', ');
    ...
    const systemPrompt: string = FOOD_METADATA_SYSTEM_PROMPT || 'You are a professional photo archivist. Your tone is informative, concise, and professional. Return ONLY a JSON object.';

    const nearbyForPrompt: any[] = state.nearby_food_places_curated || state.poiAnalysis?.food?.curated || state.nearby_food_places || state.poiAnalysis?.food?.candidates || [];
    const foodMeta = state.poiAnalysis?.food || {};
    const bestCandidate = state.best_restaurant_candidate || null;
    const deterministicRestaurant: boolean = !!foodMeta?.deterministic_restaurant || !!bestCandidate?.deterministic;
    const lockedRestaurantName: string | null = foodMeta?.restaurant_name || bestCandidate?.name || null;
    const lockedRestaurantAddress: string | null = foodMeta?.restaurant_address || bestCandidate?.address || null;

    let userPrompt: string = (FOOD_METADATA_USER_PROMPT || '')
    ...
    const configuredModel: string = state.modelOverrides?.defaultModel || 'gpt-4o-mini';
    ...
    const finalParsed: Record<string, any> = {
    ...
    const chosenPlaceId: string | null = parsed.chosen_place_id || parsed.chosen_placeId || parsed.restaurant_place_id || null;
    let chosenCandidate: any = null;
    if (chosenPlaceId) {
      const candidateSource: any[] = state.nearby_food_places_curated || state.nearby_food_places || [];
      if (Array.isArray(candidateSource)) {
        chosenCandidate = candidateSource.find((p: any) => p.placeId === chosenPlaceId || p.place_id === chosenPlaceId) || null;
      }
    }
    const parsedRestaurantConfidence: number | null = parseNumber(parsed.restaurant_confidence);
    const impliedConfidence: number = parsedRestaurantConfidence != null ? parsedRestaurantConfidence : 0.85;
    ...
    const enforcedDescription: string = ensureRestaurantInDescription(finalParsed.description, foodData.restaurant_name, locationString, timestamp);
    finalParsed.description = enforcedDescription;
    foodData.description = enforcedDescription;

    let nutrition: any = parsed.nutrition_info || null;
    let nutrition_conf: number = parseNumber(parsed.nutrition_confidence) || 0.0;
    if (!nutrition && (foodData.restaurant_name || foodData.dish_name)) {
      try {
        ...
        if (searchResult) {
          ...
          try {
            const estRaw: string = estimateResp?.choices?.[0]?.message?.content || '{}';
    ...
    let bestCandidateObj: any = state.best_restaurant_candidate || null;
    if (chosenCandidate && impliedConfidence >= 0.5) {
