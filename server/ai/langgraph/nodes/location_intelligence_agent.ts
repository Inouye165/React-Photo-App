...
async function location_intelligence_agent(state: Record<string, any>): Promise<Record<string, any>> {
  ...
  const gpsString: string | null = coordinates
    ? `${coordinates.lat.toFixed(6)},${coordinates.lon.toFixed(6)}`
    : state.gpsString || null;
  const heading: number | null = extractHeading(metadata);
  const altitude: number | null = extractAltitude(metadata);
  const timestamp = extractTimestamp(metadata);
  const headingCardinal: string | null = headingToCardinal(heading);
  ...
  if (coordinates) {
    ...
    try {
      const skipGeneric: boolean = shouldSkipGenericPoi(classificationLower);
      if (skipGeneric) {
        nearby = [];
      } else {
    ...
    try {
      ...
      if (skipGeneric) {
        ...
        const foodCandidates: NearbyPlace[] = Array.isArray(nearby)
          ? nearby.filter((p: NearbyPlace) => Array.isArray(p.types) && p.types!.some((t: string) => FOOD_TYPES.includes(t)))
          : [];

        const typedCandidates: NearbyPlace[] = foodCandidates.length
          ? foodCandidates
          : Array.isArray(nearby)
          ? nearby.filter((p: NearbyPlace) => (p.category || '').toLowerCase() === 'restaurant')
          : [];

        const curated: NearbyPlace[] = (typedCandidates.length ? typedCandidates : nearby || [])
          .slice()
        ...
        let preselected: NearbyPlace | null = null;
        if (curated.length > 0) {
          const top: NearbyPlace = curated[0];
        ...
        try {
          ...
          const nearbyLen: number = Array.isArray(state.nearby_food_places) ? state.nearby_food_places.length : 0;
          const src: string | null = (best && best.source) || null;
          const rating: number = Number(best?.rating || 0);
  ...
  const locationModel: string = state.modelOverrides?.locationModel || 'gpt-4o-mini';
  const systemPrompt: string = LOCATION_INTEL_SYSTEM_PROMPT;

  const userPrompt: string = LOCATION_INTEL_USER_PROMPT.replace('{structuredContext}', JSON.stringify(structuredContext, null, 2));

  let locationIntel: LocationIntel;
  try {
    ...
    const fallback = buildLocationIntelDefaults({
      ...
      nearest_park: sanitizeIntelField(nearby.find((place: NearbyPlace) => (place?.category || '').toLowerCase() === 'park')?.name) || 'unknown',
      nearest_trail: sanitizeIntelField(osmTrails?.[0]?.name) || sanitizeIntelField(nearby.find((place: NearbyPlace) => (place?.category || '').toLowerCase() === 'trail')?.name) || 'unknown',
...
export = location_intelligence_agent;
