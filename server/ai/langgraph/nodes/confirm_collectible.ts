...
function isoPlusHours(hours: number): string {
  const ms: number = Number(hours) * 60 * 60 * 1000;
  return new Date(Date.now() + ms).toISOString();
}

function safeTrimString(val: unknown): string | null {
 ...
 */
async function confirm_collectible(state: Record<string, any>): Promise<Record<string, any>> {
  const runId: string | null = state.runId || null;
  ...
  if (state.collectibleOverride && safeTrimString(state.collectibleOverride.id)) {
    ...
    const confirmedBy: string = safeTrimString(override.confirmedBy) || 'user';
    ...
    const history: any[] = Array.isArray(current.review?.editHistory) ? current.review.editHistory : [];
  ...
  const confidence: number | null = typeof identification.confidence === 'number' ? identification.confidence : null;
  const ticketId: string | null = current.review?.ticketId || runId;
