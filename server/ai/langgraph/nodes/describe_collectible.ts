/**
 * describe_collectible.ts - LangGraph Node
 * 
 ...
 */
async function describe_collectible(state: Record<string, any>): Promise<Record<string, any>> {
  try {
    ...
    const searchResults: any[] = state.collectibleSearchResults || [];
    
    // Also include market_data from valuation if available (Sprint 2)
    const marketData: any[] = valuation?.market_data || [];
    
    // Parse search results to extract actual URLs and snippets
    const formattedSearchResults: FormattedSearchResult[] = searchResults.map((result: any) => {
      try {
    ...
    const allSearchResults = formattedSearchResults.flatMap((sr: FormattedSearchResult) => sr.results);
...
${allSearchResults.length > 0 ? allSearchResults.map((r: any, i: number) => `
...
Create an engaging description that a collector would appreciate.`;
    ...
    try {
      ...
      try {
        ...
        const cat: string = typeof analysisContext.category === 'string' ? analysisContext.category : (analysisContext.category?.value || 'Item');
        const cond: string = analysisContext.condition?.label || analysisContext.condition?.value?.label || 'Unknown';
        const valMin = analysisContext.value?.min || '?';
        const valMax = analysisContext.value?.max || '?';
        const valCurr: string = analysisContext.value?.currency || 'USD';
