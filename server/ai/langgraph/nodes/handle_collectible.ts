...
interface ToolNodeLike {
  invoke: (messages: any[]) => Promise<any[]>;
}

function createToolNode(): ToolNodeLike {
...
const toolNode: ToolNodeLike = createToolNode();

interface AgentLoopResult {
  output: string;
  intermediateSteps: Array<{ action: { tool: string }; observation: any }>;
}
 ...
 */
async function runAgentLoop(messages: any[], runId: string | null): Promise<AgentLoopResult> {
  const intermediateSteps: Array<{ action: { tool: string }; observation: any }> = [];
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    if (runId) {
      const modelName: string = response.response_metadata?.model_name || 'unknown-model';
  ...
  if (runId) {
    const modelName: string = finalResponse.response_metadata?.model_name || 'unknown-model';
 ...
 */
async function handle_collectible(state: Record<string, any>): Promise<Record<string, any>> {
  try {
...
Then provide your final answer as a VALID JSON object matching the Schema. Do NOT include any markdown formatting or code blocks.`
    ...
    const response: AgentLoopResult = await runAgentLoop(messages, state.runId);
    ...
    if (response.intermediateSteps && response.intermediateSteps.length > 0) {
      logger.info('[LangGraph] handle_collectible: Agent used tools', {
        ...
        tools: response.intermediateSteps.map((step: any) => step.action?.tool || 'unknown')
      });
    }
    ...
    try {
      ...
      let cleanOutput: string = response.output;
    ...
    if (!validationResult.success) {
      ...
      return {
        ...
        collectibleResult: {
          collectibleData: null,
          status: 'failed',
          error: `Schema validation failed: ${validationResult.error.errors.map((e: any) => `${e.path.join('.')}: ${e.message}`).join('; ')}`
        }
      };
    }
    ...
    const searchResults = intermediateSteps
      .filter((step: any) => step.action?.tool)
      .map((step: any) => ({
