...
} = require('../prompts/food_metadata_agent');
...
async function classify_image(state) {
  try {
    // Only log minimal metadata: GPS, direction, date/time, device
    const meta = state.metadata || {};
    const gps = meta?.latitude && meta?.longitude ? `${meta.latitude},${meta.longitude}` : null;
      const headingLog = extractHeading(meta) || null;
    const date = meta.DateTimeOriginal || meta.dateTime || null;
    const device = meta.cameraModel || meta.Make || meta.Model || null;
      logger.info('[classify_image] gps=%s, heading=%s, date=%s, device=%s', gps, headingLog, date, device);
    const prompt =
...
async function generate_metadata(state) {
  try {
    ...
    const date = meta.DateTimeOriginal || meta.dateTime || null;
    const device = meta.cameraModel || meta.Make || meta.Model || null;
      logger.info('[generate_metadata] gps=%s, heading=%s, date=%s, device=%s', gps, headingLog, date, device);
...
async function location_intelligence_agent(state) {
  ...
  // Only log minimal metadata: GPS, direction, date/time, device
  const meta = state.metadata || {};
  const gps = coordinates ? `${coordinates.lat.toFixed(6)},${coordinates.lon.toFixed(6)}` : null;
    const headingLog = extractHeading(meta) || null;
  const date = meta.DateTimeOriginal || meta.dateTime || null;
  const device = meta.cameraModel || meta.Make || meta.Model || null;
    logger.info('[location_intel] gps=%s, heading=%s, date=%s, device=%s', gps, headingLog, date, device);
...
async function food_metadata_agent(state) {
  ...
  try {
    ...
    const date = meta.DateTimeOriginal || meta.dateTime || null;
    const device = meta.cameraModel || meta.Make || meta.Model || null;
      logger.info('[food_metadata_agent] gps=%s, heading=%s, date=%s, device=%s', gps, headingLog, date, device);
  ...
  const region = promptPoi.region || (promptPoi.locationIntel ? promptPoi.locationIntel.region : null) || null;
    ...
    const systemPrompt = FOOD_METADATA_SYSTEM_PROMPT;
    ...
    let userPrompt = FOOD_METADATA_USER_PROMPT
      .replace('{classification}', String(state.classification || 'unknown'))
      ...
      .replace('{CRITICAL_RULE_TEXT}', FOOD_METADATA_CRITICAL_RULE_TEXT);
    // The prompt constant already contains the critical rule insert; this
    // block remains for backward compatibility with existing code that
    // expected a safe-path to ensure the rule is present.
    ...
    userPrompt = userPrompt.replace(/`deterministic_restaurant`/g, "'deterministic_restaurant'")
      .replace(/`restaurant_name`/g, "'restaurant_name'")
      .replace(/`restaurant_address`/g, "'restaurant_address'");
