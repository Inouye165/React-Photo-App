...
const TEST_USER_ID: string = 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee';

// --- Mock DB ---
const mockReturning: jest.Mock = jest.fn();
const mockInsert: jest.Mock = jest.fn().mockReturnValue({ returning: mockReturning });
const mockSelect: jest.Mock = jest.fn();
const mockOffset: jest.Mock = jest.fn().mockReturnValue({ select: mockSelect });
const mockLimit: jest.Mock = jest.fn().mockReturnValue({ offset: mockOffset });
const mockOrderBy: jest.Mock = jest.fn().mockReturnValue({ limit: mockLimit });
const mockWhere: jest.Mock = jest.fn().mockReturnValue({ orderBy: mockOrderBy });
...
jest.mock('../middleware/auth', () => ({
  ...
  requireRole: () => (_req: any, _res: any, next: Function) => next(),
}));
...
describe('POST /api/v1/activity', () => {
  ...
  ])('accepts action "%s"', async (action: string) => {
