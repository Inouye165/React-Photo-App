...
interface CollectibleIdentification {
  id: string;
  category: string;
  confidence: number;
...
interface CollectibleState {
  collectible: {
    identification: CollectibleIdentification;
...
describe('Sprint 2: Collectibles Market Data Persistence', () => {
  describe('valuate_collectible node', () => {
    const valuate_collectible: (state: CollectibleState) => Promise<ValuationResult> =
      require('../ai/langgraph/nodes/valuate_collectible');
    ...
    test('should parse market_data array from LLM response', async () => {
      ...
      const result: ValuationResult = await valuate_collectible(state);
    ...
    test('should sanitize prices with $ and commas', async () => {
      ...
      const result: ValuationResult = await valuate_collectible({
        collectible: {
          identification: {
            id: 'Rare Item',
            category: 'Toys',
  ...
  describe('collectiblesService.addMarketDataBulk', () => {
    test('should export addMarketDataBulk function', () => {
      const createCollectiblesService: (deps: { db: jest.Mock }) => Record<string, Function> =
        require('../services/collectiblesService');
      
      // Create a mock db for testing service export
      const mockDb: jest.Mock = jest.fn(() => ({
  ...
  describe('Market data transformation logic', () => {
    test('should correctly transform AI result market_data for DB insertion', () => {
      const testUserId: string = 'test-user-uuid';
      const collectibleId: number = 123;
      ...
      const marketDataRecords: MarketDataRecord[] = marketData
        .filter((item): item is MarketDataItem & { price: number } =>
          item != null && typeof item.price === 'number' && !Number.isNaN(item.price))
        .map((item) => ({
    ...
    test('should filter out records with invalid prices', () => {
      const marketData: MarketDataItem[] = [
        { price: 100, venue: 'Valid', url: null, date_seen: null },
        { price: NaN, venue: 'Invalid NaN', url: null, date_seen: null },
        { price: 'not a number', venue: 'Invalid String', url: null, date_seen: null },
        { price: null, venue: 'Invalid Null', url: null, date_seen: null },
        { price: 200, venue: 'Also Valid', url: null, date_seen: null }
      ];

      const filtered: MarketDataItem[] = marketData.filter(
        (item): item is MarketDataItem =>
          item != null && typeof item.price === 'number' && !Number.isNaN(item.price)
      );
    ...
    test('should handle missing market_data gracefully', () => {
      ...
      const marketData: MarketDataItem[] | undefined = collectibleInsights?.valuation?.market_data;
      const hasMarketData: boolean = Array.isArray(marketData) && marketData.length > 0;

      expect(hasMarketData).toBe(false);
    });

    test('should truncate long venue names and reject long URLs', () => {
      const longVenue: string = 'A'.repeat(300);
      const longUrl: string = 'https://example.com/' + 'a'.repeat(2100);

      const marketData: Array<{ price: number; venue: string; url: string }> = [
        { price: 100, venue: longVenue, url: longUrl }
      ];

      const transformed: Array<{ venue: string | null; url: string | null }> = marketData.map(
        (item) => ({
          venue: item.venue ? String(item.venue).substring(0, 255) : null,
          url: (item.url && typeof item.url === 'string' && item.url.length < 2048) ? item.url : null
        })
