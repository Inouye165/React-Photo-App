...
describe('Authentication Security Tests', () => {
  ...
  beforeAll(() => {
    ...
    app.get('/protected', authenticateToken, (req: any, res: any) => { // lgtm[js/missing-rate-limiting] - Test-only mock server
  ...
  describe('CWE-489: Active Debug Code Remediation', () => {
    ...
    test('CRITICAL: Debug routes should reject requests even when ALLOW_DEV_DEBUG is true', async () => {
      ...
      const originalAllowDevDebug: string | undefined = process.env.ALLOW_DEV_DEBUG;
    ...
    test('CRITICAL: should reject mock-token even when MOCK_AUTH is set', async () => {
      // Store original value
      const originalMockAuth: string | undefined = process.env.MOCK_AUTH;
    ...
    test('SECURITY: should not have MOCK_AUTH code paths in production logic', () => {
      ...
      const authRoutesPath: string = path.join(__dirname, '../routes/auth.ts');
      
      const authMiddlewareContent: string = fs.readFileSync(authMiddlewarePath, 'utf8');
      const authRoutesContent: string = fs.readFileSync(authRoutesPath, 'utf8');
