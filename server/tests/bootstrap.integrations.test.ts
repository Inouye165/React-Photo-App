...
describe('bootstrap/startIntegrations + shutdown', () => {
  ...
  function createKnexStub(): any {
    const target: jest.Mock = jest.fn(() => target);
    return new Proxy(target, {
      get(obj: any, prop: string | symbol): any {
        if (prop in obj) return obj[prop];
        return jest.fn(() => obj);
      },
      apply(obj: any, thisArg: any, args: any[]): any {
        return obj(...args);
      },
    });
  }

  function createSupabaseStub(): Record<string, any> {
    return {
      storage: {
        from: jest.fn(() => ({
          upload: jest.fn(),
  ...
  test('does not start realtime subscriber on import; starts only via startIntegrations; stops via shutdown hook', async () => {
    const start: jest.Mock = jest.fn(async () => ({ started: true }));
    const stop: jest.Mock = jest.fn(async () => undefined);

    const createPhotoStatusSubscriber: jest.Mock = jest.fn(() => ({ start, stop }));
