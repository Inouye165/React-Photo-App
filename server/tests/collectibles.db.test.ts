 ...
 * 
 * @module server/tests/collectibles.db.test.ts
 */
...
interface StoreData {
  ...
  collectible_market_data: Record<string, any>[];
  collectible_photos: Record<string, any>[];
...
describe('Collectibles Database Tests', () => {
  let db: jest.Mock;
  let collectiblesService: any;
  let store: StoreData;
  
  // Test user IDs (will be created/mocked per test)
  const aliceId: string = '11111111-1111-1111-1111-111111111111';
  const bobId: string = '22222222-2222-2222-2222-222222222222';
  ...
  const createMockDb = (): jest.Mock => {
    ...
    const createQueryBuilder = (tableName: string) => {
      ...
      let gtConditions: { field: string; value: any }[] = [];
      let ltConditions: { field: string; value: any }[] = [];
      
      const applyFilter = (items: Record<string, any>[]): Record<string, any>[] => {
      ...
      const builder: Record<string, any> = {
        ...
        where: jest.fn((cond: any, op?: string, val?: any) => {
        ...
        insert: jest.fn((data: Record<string, any>) => {
          ...
          const item = { id, ...data };
          (store as any)[tableName] = (store as any)[tableName] || [];
          (store as any)[tableName].push(item);
        ...
        del: jest.fn(async () => {
          const items = (store as any)[tableName] || [];
          const initialLength: number = items.length;
          (store as any)[tableName] = items.filter((item: Record<string, any>) => {
            for (const [key, value] of Object.entries(conditions)) {
              if (item[key] !== value) return true;
            }
            return false;
          });
          return initialLength - (store as any)[tableName].length;
        }),
    ...
    return jest.fn((tableName: string) => createQueryBuilder(tableName));
  };
  ...
  describe('Feature Flags', () => {
    ...
    it('should throw when feature is disabled', () => {
      // Temporarily disable
      const originalValue: string | undefined = process.env.ENABLE_COLLECTIBLES_DB;
  ...
  describe('Cascade Delete Behavior', () => {
    it('should delete collectible when calling deleteCollectible', async () => {
      ...
      // Delete it
      const deleted: boolean = await collectiblesService.deleteCollectible(aliceId, result.id);
    ...
    it('should return false when deleting non-existent collectible', async () => {
      const deleted: boolean = await collectiblesService.deleteCollectible(aliceId, 9999);
  ...
  describe('RLS Security - User Isolation', () => {
    ...
    it('should prevent Bob from seeing Alice collectibles in list', async () => {
      ...
      expect(aliceList.every((item: any) => item.user_id === aliceId)).toBe(true);
    ...
    it('should prevent Bob from deleting Alice collectibles', async () => {
      ...
      const deleted: boolean = await collectiblesService.deleteCollectible(bobId, result.id);
  ...
  describe('AI History Append Logic', () => {
    it('should start with empty history when recordAi is false', async () => {
      ...
      const history: any[] = JSON.parse(result.ai_analysis_history);
    ...
    it('should preserve full history through multiple operations', async () => {
      ...
      expect(history.map((h: any) => h.run)).toEqual([1, 2, 4]);
    });
  });
  ...
  describe('Service Methods', () => {
    ...
    it('should filter collectibles by category', async () => {
      ...
      expect(comics.every((c: any) => c.category === 'Comics')).toBe(true);
    });
  });
});
