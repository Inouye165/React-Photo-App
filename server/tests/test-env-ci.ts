...
const CI_MODE: boolean = process.env.CI === 'true' || process.env.CI === '1';
...
if (CI_MODE) {
  ...
  const WARN_SUPPRESS: string[] = [
    'Suspicious request detected',
    'Security error from'
  ];

  const ERROR_SUPPRESS: string[] = [
  ...
  const LOG_SUPPRESS: string[] = [
    '[infer_poi]',
    '[VisualSearch]'
  ];

  console.error = (...args: unknown[]): void => {
    const flat: string = args.map((a: unknown) => {
