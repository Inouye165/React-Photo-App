...
describe('Bearer Token Authentication - Primary Auth Method', () => {
  ...
  beforeEach(() => {
    ...
    app.get('/api/test/protected', authenticateToken, (req: any, res: any) => {
      res.json({
        success: true,
        user: req.user,
  ...
  describe('Invalid/Malformed Bearer Token', () => {
    ...
    it('should NOT include actual token in error response', async () => {
      const sensitiveToken: string = 'super-secret-sensitive-token-12345';
      ...
      const responseText: string = JSON.stringify(response.body);
...
describe('CORS Configuration - Authorization Header', () => {
  it('should allow Authorization header in CORS preflight', async () => {
    ...
    app.options('/api/test', (req: any, res: any) => res.sendStatus(204));
