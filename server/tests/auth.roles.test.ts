...
describe('Role Security Tests - Privilege Escalation Prevention', () => {
  ...
  beforeAll(() => {
    ...
    app.get('/admin/dashboard', authenticateToken, requireRole('admin'), (req: any, res: any) => { // lgtm[js/missing-rate-limiting] - Test-only mock server
      res.json({ 
        success: true, 
    ...
    app.get('/user/profile', authenticateToken, requireRole('user', 'admin'), (req: any, res: any) => { // lgtm[js/missing-rate-limiting] - Test-only mock server
      res.json({ 
        success: true, 
  ...
  describe('CRITICAL: Privilege Escalation Attack Vector (The "Hacker" Scenario)', () => {
    ...
    test('SECURITY: Should default to "user" role when both metadata are empty', async () => {
      const userWithNoRoles: MockUser = {
        id: 'user-789',
    ...
    test('SECURITY: Should allow access to user endpoints even when user_metadata has fake admin role', async () => {
      const userWithFakeAdmin: MockUser = {
        id: 'user-012',
  ...
  describe('CRITICAL: Secure Admin Access (Legitimate Admin)', () => {
    test('SECURITY: Should ALLOW access when app_metadata says "admin"', async () => {
      ...
      const legitimateAdmin: MockUser = {
        id: 'admin-123',
  ...
  describe('Code Security Verification', () => {
    test('SECURITY: Auth middleware should NOT read from user_metadata.role', () => {
      ...
      const authMiddlewarePath: string = path.join(__dirname, '../middleware/auth.ts');
      const authMiddlewareContent: string = fs.readFileSync(authMiddlewarePath, 'utf8');
      
      // Verify the vulnerable pattern is NOT present
      // The code should NOT have: user.user_metadata?.role
      const vulnerablePattern: RegExp = /user\.user_metadata\?\.\s*role/;
      const hasVulnerability: boolean = vulnerablePattern.test(authMiddlewareContent);
      
      expect(hasVulnerability).toBe(false);
      
      // Verify the secure pattern IS present
      // The code SHOULD have: user.app_metadata?.role
      const securePattern: RegExp = /user\.app_metadata\?\.\s*role/;
      const hasSecureFix: boolean = securePattern.test(authMiddlewareContent);
    ...
    test('DOCUMENTATION: Should have comment explaining why app_metadata is used', () => {
      ...
      const hasSecurityComment: boolean = /SECURITY.*app_metadata/i.test(authMiddlewareContent) ||
