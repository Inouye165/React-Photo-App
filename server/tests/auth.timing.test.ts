...
describe('Auth Timing Attack Mitigation', () => {
  const validUsername: string = 'timingtestuser';
  const validPassword: string = 'Password123!';
  const invalidUsername: string = 'nonexistentuser';
  ...
  it('should have similar response times for valid and invalid usernames', async () => {
    ...
    let totalValidTime: number = 0;
    let totalInvalidTime: number = 0;

    for (let i: number = 0; i < iterations; i++) {
      // Measure valid user + wrong password
      const startValid: number = performance.now();
      await request(app)
        .post('/auth/login')
        .send({ username: validUsername, password: 'WrongPassword123!' });
      const endValid: number = performance.now();
      totalValidTime += (endValid - startValid);

      // Measure invalid user
      const startInvalid: number = performance.now();
      await request(app)
        .post('/auth/login')
        .send({ username: invalidUsername, password: 'WrongPassword123!' });
      const endInvalid: number = performance.now();
      totalInvalidTime += (endInvalid - startInvalid);
    }

    const avgValidTime: number = totalValidTime / iterations;
    const avgInvalidTime: number = totalInvalidTime / iterations;
    const difference: number = Math.abs(avgValidTime - avgInvalidTime);
