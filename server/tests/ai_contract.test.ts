...
/**
 * AI Contract Tests - Sprint 2 + Sprint 5
 * 
 ...
 * 
 * CRITICAL: These tests mock the collectibleAgent - no real OpenAI API calls.
 * 
 * @module server/tests/ai_contract.test.ts
 */
...
// Mock the collectibleAgent and related imports before requiring the node
const mockAgentInvoke: jest.Mock = jest.fn();
jest.mock('../ai/langchain/agents', () => ({
  collectibleAgent: {
    invoke: mockAgentInvoke
  },
  collectibleTools: [],
  COLLECTIBLE_SYSTEM_PROMPT: 'Test system prompt'
}));

// Mock the ToolNode from langgraph
...
jest.mock('@langchain/core/messages', () => ({
  ...
  AIMessage: jest.fn().mockImplementation((content: string) => ({ content, role: 'assistant' }))
}));
  ...
  describe('Zod Schemas', () => {
    describe('ConditionSchema', () => {
      test('accepts valid condition values', () => {
        ...
        validConditions.forEach((condition: { rank: number; label: string }) => {
  ...
  describe('handle_collectible Node', () => {
    ...
    test('returns skipped status when feature flag is disabled', async () => {
      ...
      jest.mock('../ai/langchain/agents', () => ({
        ...
        collectibleTools: [],
        COLLECTIBLE_SYSTEM_PROMPT: 'Test prompt'
      }));
      jest.mock('@langchain/langgraph/prebuilt', () => ({
        ToolNode: jest.fn().mockImplementation(() => ({ invoke: jest.fn().mockResolvedValue([]) }))
      }));
      jest.mock('@langchain/core/messages', () => ({
        HumanMessage: jest.fn().mockImplementation((c: string) => ({ content: c, role: 'human' })),
        SystemMessage: jest.fn().mockImplementation((c: string) => ({ content: c, role: 'system' })),
        AIMessage: jest.fn().mockImplementation((c: string) => ({ content: c, role: 'assistant' }))
      }));
      jest.mock('../logger', () => ({
    ...
    test('Valid Schema Test - processes valid AI response correctly', async () => {
      // Mock agent returning response without tool calls (final answer)
    ...
    test('handles unexpected errors gracefully', async () => {
      mockAgentInvoke.mockRejectedValueOnce(new Error('Network timeout'));
