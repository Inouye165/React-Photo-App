...
async function copyExifMetadata(sourcePath, destPath) {
  try {
    ...
    logger.info(`âœ“ Copied EXIF metadata: ${path.basename(sourcePath)} â†’ ${path.basename(destPath)}`);
    return true;
  } catch (error) {
    logger.error(`âœ— Failed to copy EXIF metadata: ${error.message}`);
    return false;
  }
}
...
async function removeExifOrientation(destPath) {
  try {
    ...
    logger.info('âœ“ Removed EXIF orientation from edited file');
    return true;
  } catch (error) {
    logger.warn('âœ— Failed to remove EXIF orientation:', error.message);
    return false;
  }
}
