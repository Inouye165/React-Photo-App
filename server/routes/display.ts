...
const { validateSignedUrl } = require('../utils/urlSigning');
...
module.exports = function createDisplayRouter({ db }) {
  ...
  function routeSignedThumbnailRequests(req, _res, next) {
    const { sig, exp } = req.query || {};
    if (!(sig && exp)) {
      return next('route');
    }

    return validateSignedUrl(req, _res, () => {
      req.user = null;
      next();
    });
  }
  ...
  // Signed thumbnail URLs are handled on a dedicated route first.
  // Requests without signature parameters fall through to the session-auth route.
  router.get('/thumbnails/:filename', routeSignedThumbnailRequests, handleThumbnailRequest);

  // Unsigned thumbnail access always requires normal image authentication.
  router.get('/thumbnails/:filename', authenticateImageRequest, handleThumbnailRequest);
