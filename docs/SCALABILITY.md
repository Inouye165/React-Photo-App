...
- **Display Endpoints:** Photo, chat-image, and thumbnail display endpoints now prefer **HTTP redirects to short-lived Supabase Storage signed URLs** (offloading image bytes from the Node.js hot path). They fall back to server-side streaming on redirect/signed-URL errors. HEIC/HEIF continues to be downloaded and buffered for server-side conversion to JPEG.
