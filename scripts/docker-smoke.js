...
async function main() {
  ...
  const _proc = spawn('docker', ['run', '-d', '-p', '3001:3001', '--name', 'photo-app-smoke', 'photo-app-server:smoke'], { stdio: 'inherit' });
...
function cleanup() {
  try { execSync('docker rm -f photo-app-smoke', { stdio: 'inherit' }); } catch {}
}
