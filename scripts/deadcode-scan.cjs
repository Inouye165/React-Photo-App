...
function listFiles() {
  ...
  return out.split('\n').map(s => s.trim()).filter(Boolean).filter(p => exts.includes(path.extname(p)));
}
...
function main() {
  ...
  console.log('Top 10 files with highest incoming references:');
  const byIn = Object.keys(graph).map(f => ({ f, in: graph[f].in.size })).sort((a,b)=>b.in-a.in).slice(0,10);
  for (const x of byIn) console.log(`  ${x.f} (${x.in})`);
