...
describe('store AI polling (single poller)', () => {
  ...
  beforeEach(async () => {
    ...
    useStore.setState({
      photos: [{ id: 1, state: 'inprogress', caption: 'Processing...' }],
      pollingPhotoId: null,
  ...
  it('continues polling until API returns terminal state and then clears spinner flags', async () => {
    api.getPhoto
      .mockResolvedValueOnce({ photo: { id: 1, state: 'inprogress', caption: 'Processing...' } })
      .mockResolvedValueOnce({ photo: { id: 1, state: 'finished', caption: 'Done' } });
    ...
    const stored = useStore.getState().photos.find((p: any) => String(p.id) === '1');
  ...
  it('hard-timeout is explicit (does not silently stop while state stays inprogress)', async () => {
    api.getPhoto.mockResolvedValue({ photo: { id: 1, state: 'inprogress', caption: 'Processing...' } });
  ...
  it('stops after 5 consecutive errors (but not before)', async () => {
    api.getPhoto.mockRejectedValue(new Error('network down'));
    ...
    expect(api.getPhoto).toHaveBeenCalledTimes(5);
    expect(useStore.getState().pollingPhotoIds.has(1)).toBe(false);
    expect(useStore.getState().pollingPhotoId).toBeNull();
  });
});
