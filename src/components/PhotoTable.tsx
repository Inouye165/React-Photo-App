...
type PrivilegesMap =
  | Map<PhotoTablePhoto['id'], string>
  | Record<string, string | undefined>
  | null
  | undefined;

export interface PhotoTableProps {
  photos: PhotoTablePhoto[];
  loading: boolean;
  privilegesMap?: PrivilegesMap;
  pollingPhotoId?: PhotoTablePhoto['id'] | null;
  onSelectPhoto: (photo: PhotoTablePhoto) => void;
  onEditPhoto: (photo: PhotoTablePhoto) => void;
  onMoveToInprogress: (photoId: PhotoTablePhoto['id']) => void;
  onMoveToWorking: (photoId: PhotoTablePhoto['id']) => void;
  onDeletePhoto: (photoId: PhotoTablePhoto['id']) => void;
  apiBaseUrl?: string | null;
  getSignedUrl?: (photo: PhotoTablePhoto) => string | null | undefined;
}
...
export default function PhotoTable({
  ...
  const getAccessLevel = (photoId: PhotoTablePhoto['id']) => {
    if (!privilegesMap) return '';
    if (privilegesMap instanceof Map) return privilegesMap.get(photoId) || '';
    const key = typeof photoId === 'string' ? photoId : String(photoId);
    return privilegesMap?.[key] || '';
  };
            ...
            >
              <div className="grid grid-cols-15 gap-4 text-sm items-center">
                    ...
                    {photo.thumbnail || photo.thumbnailUrl || photo.smallThumbnailUrl ? (
                      (() => {
                        const preferredThumb =
                          photo.smallThumbnailUrl || photo.thumbnailUrl || photo.thumbnail;
                        const signedUrl = getSignedUrl ? getSignedUrl(photo) : null;
                        const imageUrl = signedUrl || toUrl(preferredThumb, apiBaseUrl ?? undefined);
                ...
                <div className="col-span-3 text-gray-600">
                  {metadata.DateTimeOriginal || metadata.CreateDate || 'Unknown'}
                </div>
                <div className="col-span-1 text-gray-600 text-xs">
                  {formatFileSize(photo.file_size)}
                </div>
                <div className="col-span-2 text-gray-600">
                  {photo.state === 'working' ? 'working' : photo.state}
                </div>
