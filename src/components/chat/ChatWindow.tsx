...
export default function ChatWindow({ roomId, showIdentityGate, mode = 'workspace' }: ChatWindowProps) {
        ...
        >
          ...
          <div className="flex-1 min-h-0 flex items-center justify-center p-6">
            <div className="max-w-md w-full text-center">
              ...
              <p className="mt-2 text-sm text-slate-600">Say hello — invite someone or send a quick message.</p>
              <div className="mt-4 flex items-center justify-center gap-3">
                <button
                  type="button"
                  onClick={() => {
                    const btn = document.querySelector('[data-testid="chat-new-message"]') as HTMLElement | null
                    if (btn) {
                      try {
                        btn.click()
                      } catch {
                  ...
                  className="inline-flex items-center justify-center rounded-2xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-900/10"
  ...
  const chatThread = (
    <section className="flex flex-col h-full min-h-0 relative" aria-label="Chat window">
      ...
      <div data-testid="chat-composer" className="border-t border-slate-200 bg-white p-4 sm:p-5 shrink-0">
        ...
        <form
          className="flex items-end gap-3 bg-white rounded-2xl border border-slate-200 shadow-sm p-3"
          onSubmit={(e) => {
            e.preventDefault()
            onSend()
          }}
        >
          ...
          <textarea
            ...
            rows={1}
            disabled={!canSend}
            className="flex-1 resize-none rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/20"
          />
          <button
