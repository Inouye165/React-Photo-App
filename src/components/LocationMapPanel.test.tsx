...
vi.mock('@vis.gl/react-google-maps', () => ({
  ...
  AdvancedMarker: ({ position, children }: AdvancedMarkerProps) => (
...
describe('LocationMapPanel component', () => {
  ...
  describe('with Google Maps API key', () => {
    ...
    it('centers map on photo coordinates', () => {
      ...
      const centerAttr = map.getAttribute('data-center');
      expect(centerAttr).not.toBeNull();
      const centerData = JSON.parse(centerAttr as string);
    ...
    it('updates map center when photo changes', () => {
      ...
      let centerAttr = map.getAttribute('data-center');
      expect(centerAttr).not.toBeNull();
      let centerData = JSON.parse(centerAttr as string);
      ...
      centerAttr = map.getAttribute('data-center');
      expect(centerAttr).not.toBeNull();
      centerData = JSON.parse(centerAttr as string);
      expect(centerData.lat).toBe(40.7128);
      expect(centerData.lng).toBe(-74.0060);
    });
  ...
  describe('without Google Maps API key (OSM fallback)', () => {
    ...
    it('uses flex column layout with flex-1 for iframe', () => {
      ...
      const iframeWrapper = outerDiv.firstChild as HTMLElement;
