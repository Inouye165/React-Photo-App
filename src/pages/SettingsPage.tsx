...
type GradingScales = Record<string, Grade[]>

function normalizeGradingScales(input: unknown): GradingScales {
  if (!input || typeof input !== 'object') return {}
  const record = input as Record<string, unknown>

  const out: GradingScales = {}
  for (const [category, maybeGrades] of Object.entries(record)) {
    if (!Array.isArray(maybeGrades)) continue

    const grades: Grade[] = maybeGrades
...
export default function SettingsPage() {
  ...
  const gradingScales = useMemo(() => normalizeGradingScales(preferences?.gradingScales), [preferences])

  // Selected category for right panel
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null)

  // Form state for adding new grade
  const [newGrade, setNewGrade] = useState<Grade>({ label: '', rank: 3, definition: '' })
  ...
  const allCategories = useMemo(() => {
    const userCategories = Object.keys(gradingScales || {})
    const merged = new Set([...defaultCategories, ...userCategories])
    return Array.from(merged).sort()
  }, [gradingScales, defaultCategories])

  // Get grades for selected category
  const selectedGrades = useMemo(() => {
    if (!selectedCategory) return []
    return gradingScales?.[selectedCategory] || []
  }, [selectedCategory, gradingScales])
  ...
  const handleAddGrade = async () => {
    ...
    setLoading(true)
    try {
      const currentGrades = gradingScales?.[selectedCategory] || []
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <button onClick={() => navigate('/gallery')} className="px-4 py-2 text-gray-600 hover:text-gray-900">
            ← Back to Gallery
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <ul className="space-y-1" data-testid="category-list">
              {allCategories.map((category) => {
                const count = gradingScales?.[category]?.length || 0
                return (
                  <li key={category}>
                    <button
            ...
            <div className="mt-6 pt-4 border-t border-gray-200">
              ...
              <p className="mt-2 text-xs text-gray-500">Adds standard grading scales for common categories</p>
            </div>
          </div>
          ...
          <div className="md:col-span-2 bg-white rounded-lg shadow p-6">
            {selectedCategory ? (
              <>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-semibold text-gray-900">{selectedCategory} Grading Scale</h2>
                  <button
                ...
                {showAddForm && (
                  <div className="mb-6 p-4 bg-gray-50 rounded-md" data-testid="add-grade-form">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Rank (1-5)</label>
                        <select
                          value={newGrade.rank}
                          onChange={(e) => setNewGrade({ ...newGrade, rank: parseInt(e.target.value, 10) })}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md"
                          disabled={loading}
                ...
                {selectedGrades.length > 0 ? (
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {[...selectedGrades]
                          ...
                          .map((grade) => {
                            const index = selectedGrades.findIndex((g) => g.label === grade.label && g.rank === grade.rank)
                            return (
                              <tr key={`${grade.label}-${grade.rank}-${index}`}>
                                ...
                                <td className="px-4 py-3 whitespace-nowrap font-medium text-gray-900">{grade.label}</td>
                                <td className="px-4 py-3 text-gray-500 text-sm">{grade.definition}</td>
                  ...
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="mt-2 text-sm">
                      Click "Add Grade" to create one, or "Load Defaults" to use standard scales.
                    </p>
                  </div>
                )}
              </>
